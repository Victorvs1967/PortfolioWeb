...
// projects filter mixer with MixItUp library ( https://www.kunkalabs.com/mixitup/ )
const mixer = mixitup(containerEl);

mainHead.style.minHeight = `${window.screen.height}px`;

// animate on scroll with AOS library ( https://michalsnik.github.io/aos/ )
AOS.init({
  duration: 1000,
  delay: 100,
});
...
// portfolio project fullscreen view
portfolioItems.forEach(item => {
  item.querySelector('a').addEventListener('click', event => {
    fadeIn(overflow, 600);
    const description = overflow.querySelector(event.target.hash);
...
// animate burger menu w/o jQuiery
menuTopItems.forEach(item => {
